...
func Test_deserializationOfString(t *testing.T) {
	command := "+OK\r\n"
	splittedCommand := bytes.Split(Byte(command), SeparatorBytes)
	parsedString, _ := parseSimple(splittedCommand)
...
func Test_DeserializationOfInteger(t *testing.T) {
	...
	parsedString, _ := parseNumber(splittedCommand)
...
func Test_DeserializationOfNilBulkString(t *testing.T) {
	command := "$-1\r\n"
	splittedCommand := bytes.Split(Byte(command), SeparatorBytes)
	parsedString, _ := parseBulkString(splittedCommand)
...
func Test_DeserializationOfEmptyArray(t *testing.T) {
	command := "*0\r\n"
	splittedCommand := bytes.Split(Byte(command), SeparatorBytes)
	result, _ := parseArray(splittedCommand)
...
func Test_DeserializationOfArray(t *testing.T) {
	command := "*3\r\n$5\r\nhello\r\n:1\r\n$5\r\nworld\r\n"
	splittedCommand := bytes.Split(Byte(command), SeparatorBytes)
	arr, _ := parseArray(splittedCommand)
...
func Test_Split(t *testing.T) {
	command := "*3\r\n:1\r\n:2\r\n:3\r\n"
	arr := bytes.Split(Byte(command), []byte{CarriageReturn, NewLine})
	fmt.Println(arr)
}
...
func Test_DeserializationOfNestedArray(t *testing.T) {
	command := "*2\r\n*3\r\n:1\r\n:2\r\n:3\r\n*2\r\n+Hello\r\n-World\r\n"
	splitCommand := bytes.Split(Byte(command), SeparatorBytes)

	result, _ := parseArray(splitCommand)
	//fmt.Println(result)
